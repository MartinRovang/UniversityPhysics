...
\begin{figure}[H]
    {\centering
        \includegraphics[width=0.75\textwidth]{removed_per_noise.pdf}
        \caption{Left; Original image, Right; Filtered image, here we managed to remove almost all the noise. We have also sharpened using a Butterworth high pass mask.}
        \label{C7F5}
    \par}
    \end{figure}

After removing the noise with the notch filter, we made a edge mask using Butterworth with cutoff 100 and order $n = 3$.
